...
## Known integration risks (resolve during execution, not blockers)
1. `control_plane.db.Database` construction in tests must match the real fixture — Task 3/5 notes say to copy `tests/test_control_plane.py`'s db fixture verbatim if the `sqlite:///` URL form differs.
2. How the real app exposes the db pool to routers (`app.state.db` vs `Depends(get_db)`) — Task 5 Step 0/6 reconcile this against the actual `include_router` site.
3. `_call_provider` signature in `gateway.py` — Task 5's `call_model` mirrors the call at `gateway.py:459`; confirm arg names if it changed.
