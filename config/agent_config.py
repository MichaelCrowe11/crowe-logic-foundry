...
_BASE_MODEL_CHAIN = [
    ...
    # ensures customer never sees upstream lineage.
    {
        ...
        "label": "CroweLM DeepParallel",
        "type": "reasoning",
        "provider": "deepparallel",
        "backend_name": "crowelm-cluster-multilineage-v1",
        "aliases": [
            "deepparallel",
            "crowelm-deepparallel",
            "dp",
            "DeepParallel",
            "CroweLM DeepParallel",
            "cluster",
    ...
    {
        "name": "crowelm-unified-v2:latest",
        "label": "CroweLM Unified Local",
        "type": "reasoning",
        "provider": "ollama",
        "backend_name": "crowelm-unified-v2:latest",
        "aliases": [
            "unified",
            "unified-local",
            "crowelm-unified",
            "crowelm-unified-v2",
            "CroweLM Unified Local",
            "deepparallel-local",
            "dp-local",
            "local-deepparallel",
    ...
    # CroweLM Unified Local Gemma 4 8B variant — narrower domain, sharper voice.
    {
        ...
        "prompt": (
            "You are CroweLM Cadence, Crowe Logic's voice tier. You speak in "
            "Michael Crowe's register: lowercase casual when chatting, technical "
            "and precise when teaching cultivation, lyrical and measured when "
            "reflecting on the Substrate body of work. No em dashes. No emojis. "
            "Trade-off-forward. Surface real constraints rather than performing "
            "optimism. When asked for instructions, give specific numbers "
            "(temperatures, humidities, ratios, days). When asked for prose, "
            "let the cadence stretch."
        ),
    },
    {
        "name": "glm-4.6:cloud",
...
_TIER_OVERLAYS: dict[str, str] = {
    ...
    "voice": (
        "## Tier behavior: voice\n"
        "Prioritize style fidelity and practical specificity over broad analysis. "
        "Keep the user's requested register intact, but do not invent personal history, "
        "private facts, or backend details. For operational answers, state the tradeoff "
        "and next action plainly."
    ),
    "instruct": (
